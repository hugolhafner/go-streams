...
type ForEachProcessor[K, V any] struct {
	action func(K, V) error
	ctx    processor.Context[K, V]
}

func NewForEachProcessor[K, V any](action func(K, V) error) *ForEachProcessor[K, V] {
...
func (p *ForEachProcessor[K, V]) Process(r *record.Record[K, V]) error {
	return p.action(r.Key, r.Value)
}
